...
inline fn parseGround(input: []const u8) !Result {
    ...
    const key: Key = switch (b) {
        ...
        0x7F => .{ .codepoint = Key.backspace },
            ...
            const first_len = std.unicode.utf8ByteSequenceLength(b) catch return error.InvalidUTF8;
            // TODO: reconsider it all
            if (input.len < first_len) return .{
                .event = null,
                .n = 0,
            };

            n = first_len;
            const code = try std.unicode.utf8Decode(input[0..first_len]); // !DEPRECATED
